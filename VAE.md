...
Instead of trying to compute the true posterior (p\_\theta(z\mid x)) exactly, we introduce a simpler approximation:
...
D*{KL}\big(q*\phi(z\mid x),|,p\_\theta(z\mid x)\big)

\mathbb{E}_{q_\phi(z\mid x)}  
\left$$  
\log \frac{q*\phi(z\mid x)}{p*\theta(z\mid x)}  
\right$$  
\ge 0
...
p*\theta(z\mid x)=\frac{p*\theta(x,z)}{p\_\theta(x)}
...
\log p*\theta(z\mid x)=\log p*\theta(x,z)-\log p\_\theta(x)  
\quad  
(\log(a/b)=\log a-\log b)
...
\big(\log p*\theta(x,z)-\log p*\theta(x)\big)  
\right$$
...
\mathbb{E}_{q_\phi}\left$$  
\log q*\phi(z\mid x)-\log p*\theta(x,z)+\log p\_\theta(x)  
\right$$
...
\mathbb{E}_{q_\phi(z\mid x)}$$\log p_\theta(x)$$ = \log p\_\theta(x)
...
Rearrange to solve for (\log p\_\theta(x)):
...
D*{KL}\big(q*\phi(z\mid x),|,p\_\theta(z)\big)
...
\nabla*\theta \mathbb{E}*{q*\phi(z\mid x)}  
\left$$\log p*\theta(x,z)-\log q\_\phi(z\mid x)\right$$
...
\nabla*\phi,\mathbb{E}*{p(e)}$$f(g_\phi(x,e))$$

\mathbb{E}_{p(e)}\left$$\nabla_\phi f(g\_\phi(x,e))\right$$
...
q\_\phi(z\mid x)=\mathcal{N}(\mu,\sigma)
...
q\_\phi(z\mid x)=\mathcal{N}(\mu,\operatorname{diag}(\sigma^2)),  
\quad  
p(z)=\mathcal{N}(0,I)
...
\boxed{  
D*{KL}\big(q*\phi(z\mid x),|,p(z)\big)

\frac{1}{2}\sum\_{j=1}^d  
...
sample (L) noises (e^{(1)},\dots,e^{(L)}\sim p(e)), build (z^{(l)}=g_\phi(x,e^{(l)})), and use:
...
\mathbb{E}_{q_\phi(z\mid x)}$$\log p_\theta(x\mid z)$$  
\approx  
\frac{1}{L}\sum*{l=1}^L \log p*\theta(x\mid z^{(l)})
...
\sum*{i=1}^N \mathcal{L}(\theta,\phi;x_i)  
\approx  
\frac{N}{M}\sum*{i=1}^M \mathcal{L}(\theta,\phi;x_i)
...
- \beta;D*{KL}\big(q*\phi(z\mid x),|,p(z)\big)  
  }  
  $$
